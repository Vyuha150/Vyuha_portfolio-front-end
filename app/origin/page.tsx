    ...
    <main className="min-h-screen text-white mt-10 md:mt-24">
      ...
      <section className="relative flex flex-col md:flex-row items-center justify-between h-auto px-4 space-y-6 md:space-y-0">
        ...
        >
          ...
          <button
            className="mt-6 px-6 py-3 bg-transparent border border-white hover:bg-orange-500 text-orange-500 font-semibold rounded-lg shadow-md hover:text-white hover:scale-105 transition-all duration-300 ease-in-out hover:border-transparent"
            onClick={handleclick}
          >
            Know More
          </button>
...
function Achievement({ title, description, image, reverse }: any) {
  ...
  return (
    <motion.div
      ...
      } items-start md:items-center space-y-6 md:space-y-10 mb-10`}
    >
      {/* Content */}
      <div
        className={`md:w-1/2 space-y-4 flex flex-col justify-center items-start md:items-start ${
          reverse ? "md:pl-6" : "md:pr-6"
        }`}
      >
        <h3 className="text-2xl font-bold text-orange-400 text-start md:text-start">
          {title}
        </h3>
      ...
      {/* Circle */}
      <div className="absolute left-0 md:left-1/2 transform -translate-x-1/2 flex items-start justify-center top-0">
        <div
